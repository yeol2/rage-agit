...
// 관리자가 명단 옮기던 그 밤을 기억하며.
const HOW_STEPS: HowStep[] = [
  {
    title: '명단 등록',
    body: '관리자가 디스코드 명단을 긁어서 여기 업로드하면 끝. 클랜원은 로그인은커녕 아무것도 안 눌러도 됩니다 — 그래서 비밀번호나 로그인 정보는 애초에 받을 수가 없어요.',
    note: '힘들었음',
  },
  {
    title: '내전 자동 수집',
...
export const siteConfig = {
  ...
  hero: {
    ...
    bodyLines: [
      '가입? 등록? 그런 거 없어요 — 관리자가 이미 다 긁어놨습니다.',
      '최근 내전 4회 동안 얼마나 잘했는지, 킬·데미지·순위·생존율로 쪼개서 확인합니다.',
    ],
    ctaLabel: '리더보드 보기',
    ctaHref: '/dashboard',
  },
  ...
  memberDirectory: {
    ...
    hexagon: {
      heading: '6각형 지표',
      // 두 줄까지만. 그림 옆에 붙는 글이라 길어지면 그림보다 먼저 읽힌다.
      lines: [
        '점선은 내 티어 그룹의 평균입니다. 실선이 그 바깥이면 잘한 축입니다.',
        '2026년 6월 7일 이후 경기만 집계됩니다.',
      ],
      // 맵 기록은 맵 이름이 남은 경기만 셀 수 있다 — 그보다 이전 내전은 결과
    // 스크린샷뿐이라 맵이 어디였는지가 기록에 없다. 그래서 위 전적 요약보다
    // 경기 수가 적게 나오는데, 그 이유를 화면에 적어두지 않으면 숫자가 어긋난
    // 것처럼 보인다.
    mapRecordsScope: '2026-06-07 이후',
    // 안정성은 여섯 축 중 유일하게 이름만 봐서는 뜻을 알 수 없어서 물음표를 단다.
      stabilityHelp:
        '역대 전체 팀등수가 얼마나 흔들렸는지(표준편차)입니다. 바깥쪽일수록 매 내전 결과가 고르고, 안쪽일수록 잘할 때와 못할 때의 차이가 큽니다.',
    },
  },
  ...
  about: {
    ...
    how: {
      ...
      heading: '어떻게 굴러가냐면.',
      intro:
        '클랜원이 할 일은 없어요. 관리자가 명단만 올려두면 그다음부터는 전부 자동입니다. 계산 방식은 복잡해서 다 안 적을게요 — 큰 흐름만 봐주세요.',
      steps: HOW_STEPS,
    },
    ...
    data: {
      ...
      closingNote: '내전은 본캐로 해주세요. 부캐까지 하는 건 귀찮아서 안 했어요.',
    },
  },
} as const;
