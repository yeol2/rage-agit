...
describe('PartnerChemistry', () => {
  ...
  it('동률이면 한 칸에 여러 명이 서고 등수 차이는 한 번만 적는다', () => {
    const twin = { ...good, partnerId: 'p-3', displayName: 'Ez_Twin', sessionsTogether: 2 };
    render(<PartnerChemistry best={[good, twin]} worst={[]} />);

    const best = screen.getByTestId('partner-best');
    expect(within(best).getByText('Ez_Yellow')).toBeInTheDocument();
    expect(within(best).getByText('Ez_Twin')).toBeInTheDocument();
    expect(within(best).getAllByText(/6\.7등/)).toHaveLength(1);
  });
  ...
  it('후보가 없는 쪽은 빈 칸이 아니라 이유를 적는다', () => {
    render(<PartnerChemistry best={[]} worst={[bad]} />);

    const best = screen.getByTestId('partner-best');
    expect(
      within(best).getByText(/내전 2회 이상 같은 팀이었던 사람이 없습니다/),
    ).toBeInTheDocument();
  });
});
