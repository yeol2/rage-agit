...
if (attendance.size > 0) {
  ...
  const names = seedArg.split(',').map((s) => s.trim()).filter(Boolean);
...
```bash
node scripts/poll-matches.mjs --seed=Ez_Code
```

`Ez_Code`를 쓰는 이유: 최근 14일 안의 내전은 08-02 한 번뿐이고, 이 사람이 거기 참가한 것이 이미 확인됐다. 씨앗 한 명이면 그 내전 4경기가 전부 발견된다.

Expected: Players 1배치 → 매치 26건 조회 → 내전 4경기(08-02) 저장, 07-31 5경기는 탈락. 1분 안에 끝난다.

인자 없이 실행하면 씨앗을 정할 수 없다는 안내와 함께 종료된다 — 조용히 전원을 훑거나 아무나 골라 넣지 않는다.
